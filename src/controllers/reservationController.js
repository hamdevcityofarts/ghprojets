...
exports.createReservation = async (req, res) => {
  try {
    ...
    // ✅ CRITIQUE: Associer explicitement l'utilisateur connecté
    const reservationData = {
      client: req.user._id, // ✅ FORCER L'ASSOCIATION
      chambre: chambreId,
      checkIn: checkInDate,
    ...
    console.log('📦 Données réservation avec user ID:', reservationData.client);

    const reservation = await Reservation.create(reservationData);

    // ✅ POPULER LES INFORMATIONS CLIENT POUR LA RÉPONSE
...
// 🔹 Récupérer toutes les réservations - CORRECTION CRITIQUE
...
// 🔹 Récupérer une réservation par ID - CORRECTION CRITIQUE
...
// 🔹 NOUVELLE FONCTION: Récupérer les réservations d'un utilisateur spécifique
...
// 🔹 CALLBACK POUR LE RETOUR DE CYBERSOURCE - AMÉLIORÉ
exports.paymentCallback = async (req, res) => {
  try {
    ...
    const reservation = await Reservation.findById(reservationId)
      .populate('chambre')
      .populate('client'); // ✅ POPULER LE CLIENT
    ...
    if (decision === 'ACCEPT') {
      ...
      console.log('👤 Client associé:', reservation.client?._id);
